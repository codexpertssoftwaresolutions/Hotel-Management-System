...
public class HallManagementDashBoard extends javax.swing.JFrame {

    private DrawerController drawer;

    public HallManagementDashBoard(ResultSet loggedUser) {
        initComponents();
        drawer = Drawer.newDrawer(this)
                .header(new DrawerHeader())
                .enableScroll(true)
                .addChild(new DrawerItem("Dashboard").build())
                .addChild(new DrawerItem("HallBookingManage").build())
                .addChild(new DrawerItem("PackageRegistration").build())
    ...
    private void setActiveTab(int tab) { // tab -> clicked tab index
        ...
        if (tab == 0) {
            ...
            HallBookingManage hallbookingmanage = new HallBookingManage();
            bottomPanel.add(hallbookingmanage, BorderLayout.CENTER);
        } else if (tab == 2) {
            HallPackageRegistration hallpackage = new HallPackageRegistration();
            bottomPanel.add(hallpackage, BorderLayout.CENTER);
        } else if (tab == 3) {
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new HallManagementDashBoard().setVisible(true);
            }
        });
    }
